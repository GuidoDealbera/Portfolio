...
const NavBar = () => {
  ...
  return (
    <nav className={`fixed w-full left-0 top-0 z-[999] ${sticky ? "bg-white/60 text-gray-900" : "text-white"} transition-all duration-200`}>
      ...
      <div className={`z-[999] md:hidden m-5`} onClick={() => setShowMenu(!showMenu)}>
        {showMenu ? 
        <X size={30} className="text-gray-900"/> : 
        <Menu size={30} className="text-gray-100"/>}
      </div>
      <div className={`md:hidden text-gray-900 absolute w-2/3 h-screen px-7 py-2 bg-white top-0 duration-300 ${showMenu ? "right-0" : "right-[-100%]"}`}>
        <ul className="flex flex-col justify-center h-full gap-10 py-2 text-lg">
        {
          menuLinks?.map((menu, i) => (
            <li key={i} className="px-6 hover:text-cyan-600" onClick={() => setShowMenu(false)}>
              <a href={menu.link}>{menu.name}</a>
            </li>
          ))
        }
